...
It launches with a fresh private configuration home that explicitly enables
live web search and workspace network access; inherited Codex configuration,
paths, initial mode, and homes cannot alter it.  Its fake-server end-to-end
test verifies the launched child's cwd and effective configuration input.
...
does not authenticate; live activation must establish a
