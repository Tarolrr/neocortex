...
install)
  ...
  # npm installs exactly this tarball into a new isolated prefix.  npm creates
  # package-lock.json; retain it and npm ls output as the resolved dependency
  # receipt.  This is not a claim that the upstream source lock was installed.
  npm install --ignore-scripts --package-lock=true --prefix "$runtime" "$runtime/codex-acp-1.11.0.tgz"
  ...
  node -e "const p=require('$runtime/node_modules/@agentclientprotocol/codex-acp/package.json'); if(!p.engines || !p.engines.node) process.exit(1)"
